...
import { createKategoriArtikelAction, KategoriArtikelDTO } from '@/actions/artikel'
import { slugify } from '@/lib/slug'
import { toast } from 'sonner'

interface QuickCreateCategoryDialogProps {
  onSuccess: (newCategory: KategoriArtikelDTO) => void
...
export function QuickCreateCategoryDialog({ onSuccess, trigger }: QuickCreateCategoryDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    const res = await createKategoriArtikelAction({ nama: nama.trim() })
    setLoading(false)
      ...
      <DialogTrigger asChild>
        {trigger || (
          <Button
            ...
            variant='ghost'
            size='sm'
            className='h-auto p-0 text-[11px] font-semibold text-primary hover:text-primary/80 hover:bg-transparent gap-1 cursor-pointer'
            title='Tambah Kategori Baru'
          >
            <Plus className='size-3' /> + Kategori Baru
          </Button>
        )}
      </DialogTrigger>
        ...
        <form onSubmit={handleSubmit}>
          <DialogHeader className='space-y-2'>
            ...
            <DialogDescription className='text-xs text-muted-foreground'>
              Kategori baru akan langsung tersimpan dan otomatis terpilih pada form artikel yang sedang Anda tulis.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className='space-y-4 py-4'>
            <div className='space-y-1.5'>
              ...
              <Input
                id='quick-cat-nama'
                placeholder='Contoh: Renungan Harian, Wawasan Jemaat, Berita...'
                value={nama}
            ...
            {nama.trim() && (
              <div className='p-2.5 rounded-md bg-muted/50 border text-xs text-muted-foreground flex items-center gap-2'>
                ...
                <div className='truncate'>
                  Slug: <span className='font-mono text-foreground font-semibold'>/artikel?kategori={slugPreview}</span>
                </div>
              </div>
            )}
          </div>
