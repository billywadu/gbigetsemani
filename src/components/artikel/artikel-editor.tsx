...
import { uploadEditorImageAction } from '@/actions/artikel'
...
export function ArtikelEditor({
  value,
  onChange,
  placeholder = 'Tuliskan isi artikel, renungan, wawasan, atau materi di sini...',
}: ArtikelEditorProps) {
  ...
  const insertImage = () => {
    ...
    const alt = imageAlt.trim() || 'Ilustrasi Artikel'
    const imgHtml = `\n<figure class="my-6">\n  <img src="${imageUrl.trim()}" alt="${alt}" class="w-full rounded-xl shadow-sm border max-h-125 object-cover" />\n  <figcaption class="text-xs text-center text-muted-foreground mt-2 italic">${alt}</figcaption>\n</figure>\n`
      ...
      {activeTab === 'edit' ? (
        <div className='p-3'>
          <textarea
            ...
            className='w-full min-h-105 p-3 text-sm font-mono bg-transparent border-0 focus:outline-none resize-y leading-relaxed text-foreground'
          />
        </div>
      ) : (
        <div className='p-6 min-h-105 prose dark:prose-invert max-w-none bg-background/50 overflow-y-auto'>
          {value.trim() ? (
            <div
            ...
            <p className='text-muted-foreground italic text-xs text-center py-12'>
              Belum ada konten untuk ditampilkan dalam pratinjau. Tuliskan artikel pada tab editor.
            </p>
          )}
        </div>
      )}
          ...
          <div className='space-y-3 py-2 text-xs'>
            ...
            <div className='space-y-1'>
              ...
              <textarea
                ...
                className='w-full p-2.5 text-xs rounded-md border bg-background min-h-25'
              />
            </div>
          </div>
          <DialogFooter>
            ...
            <Button size='sm' onClick={insertBibleCallout} className='gap-1.5'>
              <Sparkles className='size-3.5' /> Sisipkan ke Artikel
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className='max-w-md'>
          ...
          <div className='space-y-3 py-2 text-xs'>
            ...
            <div className='space-y-1'>
              ...
              <Input
                placeholder='Contoh: Ilustrasi Artikel / Dokumentasi'
                value={imageAlt}
        ...
        <DialogContent className='max-w-md'>
          ...
          <div className='space-y-3 py-2 text-xs'>
            <div className='space-y-1'>
              ...
              <Input
                placeholder='Contoh: Baca informasi selengkapnya'
                value={linkText}
