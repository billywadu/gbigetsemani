...
export async function ensureDefaultPengurusAndTiers() {
  ...
  // 4. Ensure Sample Seed Officials if table is totally empty
  const pengurusCount = await prisma.pengurusGereja.count()
  if (pengurusCount === 0) {
    let jemaat1 = await prisma.jemaat.findFirst({ where: { deletedAt: null } })
    if (!jemaat1) {
      jemaat1 = await prisma.jemaat.create({
        data: { nama: 'Pdt. Yohanes Sutanto', jenisKelamin: 'LAK_LAKI', statusJemaat: 'ACTIVE' },
      })
    }

    let jemaat2 = await prisma.jemaat.findFirst({
      where: { id: { not: jemaat1.id }, deletedAt: null },
    })
    if (!jemaat2) {
      jemaat2 = await prisma.jemaat.create({
        data: { nama: 'Ibu Pdp. Maria Sutanto', jenisKelamin: 'PEREMPUAN', statusJemaat: 'ACTIVE' },
      })
    }

    let jemaat3 = await prisma.jemaat.findFirst({
      where: { id: { notIn: [jemaat1.id, jemaat2.id] }, deletedAt: null },
    })
    if (!jemaat3) {
      jemaat3 = await prisma.jemaat.create({
        data: { nama: 'Bpk. Andreas Wicaksono', jenisKelamin: 'LAK_LAKI', statusJemaat: 'ACTIVE' },
      })
    }

    const mainTier1 = await prisma.strukturTier.findFirst({
      where: { lingkup: 'UTAMA', urutan: 1 },
    })
    const mainTier2 = await prisma.strukturTier.findFirst({
      where: { lingkup: 'UTAMA', urutan: 2 },
    })

    await prisma.pengurusGereja.createMany({
      data: [
        {
          jemaatId: jemaat1.id,
          tierId: mainTier1?.id || null,
          jabatan: 'Gembala Sidang',
          gelar: 'M.Th',
          kategori: 'PIMPINAN_UTAMA',
          level: 1,
          urutan: 1,
          bioRingkas: 'Melayani dengan kerendahan hati dan visi pemuridan yang berbuah lebat bagi kemuliaan Kristus.',
          isActivePublik: true,
          periodeAwal: 2020,
        },
        {
          jemaatId: jemaat2.id,
          tierId: mainTier1?.id || null,
          jabatan: 'Gembala Rekan & Konseling',
          gelar: 'S.Th',
          kategori: 'PIMPINAN_UTAMA',
          level: 1,
          urutan: 2,
          bioRingkas: 'Mendampingi penggembalaan jemaat, pelayanan keluarga, dan pemulihan jiwa-jiwa.',
          isActivePublik: true,
          periodeAwal: 2020,
        },
        {
          jemaatId: jemaat3.id,
          tierId: mainTier2?.id || null,
          jabatan: 'Ketua Majelis Jemaat',
          gelar: 'S.E',
          kategori: 'BPH_MAJELIS',
          level: 2,
          urutan: 3,
          bioRingkas: 'Mengkoordinasikan fungsi kelembagaan majelis dan penatalayanan operasional gereja.',
          isActivePublik: true,
          periodeAwal: 2024,
          periodeAkhir: 2026,
        },
